...
> 핵심 문제: 각자 다른 스레드에서 작업하면 project_memory가 diverge된다
...
| 구분 | 혼자 | 팀 |
|------|------|----|
| project_memory 관리 | 한 사람이 관리 | 공유 저장소(Git)에서 관리 |
...
공동 작업:
1. project_memory/ 초안 작성 (특히 shared/architecture.md)
2. 팀 전원 검토 및 합의
3. 공유 저장소(Git)에 저장
...
⚠️ 각 세션 시작 전 반드시:
- 최신 project_memory/ pull 확인 (다른 사람이 업데이트했을 수 있음)
        ↓
[팀 Layer 2 — 함께]
N개 작업마다 팀 전체 체크포인트
스레드: 🆕 새 스레드
...
## project_memory 공유 관리 규칙

```
1. project_memory/는 공유 저장소(Git)에서 관리
2. 변경 시 PR/MR로 팀 리뷰
...
- [ ] 팀 Layer 0에서 project_memory를 공동으로 합의했는가
- [ ] 작업 배분이 의존성 맵 기준으로 이루어졌는가
- [ ] 각자 작업 전에 최신 project_memory를 확인하는가
- [ ] 규칙 변경은 팀 합의를 거치는가
