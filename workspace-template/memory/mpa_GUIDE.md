...
하네스의 `project_memory/` 폴더는 템플릿만 제공한다.  
실제 파일은 각 프로젝트의 `workspace/project_memory/` 안에 생성한다.
...
│   ├── _template.md             ← 새 도메인 추가 시 복사해서 사용
│   └── [도메인명]/              ← 프로젝트에 맞게 직접 생성
│       ├── rules.md             ← 도메인 규칙 (필수)
│       └── registry.md          ← 재사용 요소 목록 (선택, 목록이 많아지면 분리)
│
└── roles/                       ← 페르소나별 프로젝트 누적 학습
    ├── _template.md             ← 새 페르소나 추가 시 복사해서 사용
...
판단 결과 "기억 필요"로 결론났을 때, 어디에 기록하는지:
