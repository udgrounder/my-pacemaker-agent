...
```
- **계획 승인** (`설계 완료 → 구현 중`): 사용자 명시적 승인 필요. plan.md 재수정 시 재승인.
...
**핵심 규칙:**
- plan.md를 채팅이 아닌 **파일로 먼저 저장**한다 (휘발 방지). 모든 태스크 plan.md에는 `## 실행 TODO` 체크리스트를 반드시 포함한다. 구현·에이전트 검증·사용자 확인 항목은 분리하고, 상태를 실제 증빙에 맞춰 동기화한다. **사용자 확인·결정·수동 실행 항목은 TODO의 마지막 섹션에만 둔다.** 규모가 큰 태스크는 같은 폴더의 `TODO.md`를 상세 증빙 원본으로 만들고 plan.md의 TODO에서 링크한다.
...
- **불일치·승인해시 없음** → agent에게 경고를 주입한다. agent는 변경 내용을 사용자에게 제시하고 복구 절차를 안내한다. 명시적으로 `MPA_GATE=block`을 설정한 환경에서만 차단한다.
- ⚠️ 복구 시 에이전트가 설계 영향 여부를 먼저 분류하지 않는다 — 변경 내용 제시 → 사용자 분류 → 실행 순서 고정. 상세: `agent_rules_detail.md` "계획 승인 재확인" 섹션.
