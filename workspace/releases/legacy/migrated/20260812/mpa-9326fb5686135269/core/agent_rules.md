...
**핵심 규칙:**
- plan.md를 채팅이 아닌 **파일로 먼저 저장**한다 (휘발 방지). 모든 태스크 plan.md에는 `## 실행 TODO` 체크리스트를 반드시 포함한다. 구현·에이전트 검증·사용자 확인 항목은 분리하고, 상태를 실제 증빙에 맞춰 동기화한다. 규모가 큰 태스크는 같은 폴더의 `TODO.md`를 상세 증빙 원본으로 만들고 plan.md의 TODO에서 링크한다.
