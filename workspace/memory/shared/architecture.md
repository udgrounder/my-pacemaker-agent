...
두 게이트는 동등한 원칙이다.  
**예외 — minor 타입:** `실패비용: minor`로 판단된 태스크는 계획 승인을 자동 처리하고, 완료 전 가역적 실행·기록을 경량화한다. 단, 결과가 사용자 의도에 맞는지 확인하는 완료 승인 확인은 사용자(또는 위임 에이전트) 확인 후에만 처리한다. 이는 사용자 요청이 아닌 설계된 경량 흐름이다.
