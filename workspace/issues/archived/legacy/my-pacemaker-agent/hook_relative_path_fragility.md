...
```
python3 .mpa/runtime/hooks/code_gate.py --agent claude
python3 .mpa/runtime/hooks/session_start.py ...
python3 .mpa/runtime/hooks/dist_sync.py
python3 .mpa/runtime/hooks/turn_end.py ...
```

Bash 도구에서 `cd /…/.mpa/runtime; <cmd>` 형태로 하위 디렉터리에 들어간 직후 PreToolUse(Write/Edit) 훅이 발동하면, 상대경로가 바뀐 cwd 기준으로 해석되어:
```
.mpa/runtime/.mpa/runtime/hooks/code_gate.py  ← 이중화
...
1. **훅 커맨드를 cwd-독립으로** — `$CLAUDE_PROJECT_DIR/.mpa/runtime/hooks/...` 절대경로 사용 (Claude Code가 제공하는 프로젝트 루트 환경변수). install.py가 settings.json 생성 시 이 형식으로 쓰도록.
