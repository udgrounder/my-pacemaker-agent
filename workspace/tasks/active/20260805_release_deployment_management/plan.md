...
- 설치된 MPA Runtime 프로젝트의 고유 설정은 `.mpa-project/config.yaml`에 둔다. 최초 설치 시 파일이 없으면 schema에 정의된 기본 프로젝트명·초기화 시각·현재 절대 경로를 생성하고, 기존 파일은 Runtime 프로젝트가 소유한다. 릴리즈가 명시한 `runtime.*` additive migration은 deploy transaction에서 누락값만 추가하며, 기존 값·주석·사용자 설정을 수정·정규화·삭제하지 않는다. `${project.name}`, `${project.root_path}`, `${project.initialized_at}`은 대상 로컬 config 값으로 해석한다.
...
- **최초 설치**는 `install.py`가 명시된 빈 대상에 `.mpa-workspace/`, `dist/workspace/`의 초기 골격, agent 진입 설정을 한 번만 설치한다. **Runtime 업데이트**는 준비된 release manifest를 입력으로 `release_manager.py deploy`가 대상 `workspace/issues/`의 수집 후보를 먼저 확인하고, Runtime 검증 성공 후 MPA `runtime.*` 설정을 additive 적용하며 config와 `.mpa-workspace`를 함께 backup·교체한다. 승인된 issue는 source 저장소로 원자 이동해 대상 원본을 정리한다. `install.py --upgrade`의 기존 전체 교체 경로는 Runtime 업데이트에 사용하지 않으며, agent 설정과 MPA가 관리하지 않는 config 값은 Runtime 프로젝트가 자체 관리한다.
...
- active release는 `workspace/releases/<release-id>/` 하나의 immutable bundle로 보관한다. bundle 안에는 `package_<release-id>.zip`, `manifest_<release-id>.json`, `note_<release-id>.md`, `release-receipt_<release-id>.json`을 둔다. JSON은 기계 검증용, Markdown은 사람용 설명이며 ZIP checksum과 압축 해제 후 asset map을 함께 검증한다. 대상별 deployment receipt는 `workspace/receipts/deployments/<target-ref>/`에 별도 보관한다.
- ZIP 배포는 임시 디렉터리로 안전하게 압축 해제하고 symlink·경로 탈출을 거부한 뒤 asset map과 `current_release`를 검증한다. bundle은 생성 후 수정하지 않으며 새로운 설명·소스 변경은 새 release ID를 만든다.
- Runtime update의 준비·교체·issue 수집·receipt 기록은 복구 가능한 transaction으로 처리한다. 실패하면 Runtime, issue 원본, collection receipt, deployment receipt/history를 일관된 상태로 복구하고 recovery/audit 정보만 남긴다.
- 운영 백업은 `backup-metadata.json`이 있는 성공 Runtime backup만 최신 3개까지 자동 정리한다. marker가 없는 사용자 디렉터리와 실패 백업은 자동 삭제하지 않는다.
- issue 생성 시에도 민감정보·머신 절대 경로를 검사하고, issue에는 `source_issue_id`, `workspace_issue_id`, `canonical_issue_key`, `occurrence`를 둔다. 중앙 receipt에는 프로젝트 절대 경로를 기록하지 않는다.
...
|---|---|---|---|
| `map-product` (`release_manager.py`, `install.py`, `MAP_PRODUCT_RULES.md`, `map-product-rules/`, source `workspace/`) | 이 저장소 | 최초 설치·release·deploy·issue 회수 정책 | 아니오 |
...
| Installation | 최초 설치에서 agent 설정·고유 config를 생성/보강한다. 이후 별도 refresh는 없으며, release manifest의 `runtime.*` additive migration만 deploy transaction에서 적용한다. |
| Project-local config | `.mpa-project/config.yaml`이 없으면 `schema_version`, project name, 초기화 시각, 절대 root path를 생성한다. 있으면 schema registry의 누락 필드만 원자적으로 추가하고 기존 값·주석·순서를 보존한다. release의 `runtime_config`는 `runtime.*` scalar 누락값만 추가하며 `${project.*}` 참조를 로컬 값으로 해석한다. 잘못된 값·지원하지 않는 미래 schema는 파일을 건드리지 않고 경고한다. semantic checksum은 audit/receipt에만 기록하며 절대 경로 자체는 기록하지 않는다. |
...
| 3 | Installation 계약 | 설치 골격·agent spec·고유 config schema | 구조화된 dry-run, config additive plan, smoke 결과 | 기존 설치·고유 설정·사용자 영역을 변경하지 않는 테스트 |
...
- installation-local config ownership: `.mpa-project/config.yaml`은 대상 Runtime 프로젝트의 고유 설정이다. 최초 설치에서 기본 필드를 생성하고, release manifest의 `runtime_config`가 있을 때만 `runtime.*` 누락값을 additive 적용한다. 기존 값·주석·순서·사용자 지정 빈 값은 변경하지 않으며 별도 refresh 절차는 없다.
...
| `install.py` | 구조화 dry-run, 고유 config additive initializer, smoke, 최초 설치 계약 보완 |
...
- installation이 기존 설정을 바꿈 → 기존 설치는 거부하고, Runtime 프로젝트 설정은 프로젝트 자체 절차로 관리한다.
- 고유 config를 자동 정규화하다가 사용자의 값을 바꿈 → 구조적 additive-only merge와 invalid/future schema warn-only 정책으로 구현 단계에 포함한다.
- 절대 경로가 저장소·receipt·issue로 유출됨 → migration metadata에는 `${project.root_path}` 참조만 허용하고 실제 절대 경로는 대상 local config와 backup에만 기록하며 manifest/issue/central receipt에는 기록하지 않는 검증을 구현 단계에 포함한다.
...
- [x] Release: 단일 release ID↔manifest↔receipt↔package 일대일성, argv validation의 성공·실패·timeout, active/legacy migration, schema/receipt audit을 검증한다.
- [x] Deployment: valid `from_release → to_release` dry-run과 issue collection inventory만 apply하고, 대상 외 파일 보존·post-deploy verification·issue 원자 수집/원본 정리·rollback·실패 history/receipt를 검증한다.
...
- [x] `README.md` — release bundle 경로, issue 수집 보존, Git의 선택적 역할을 추가
- [x] `guidebook/guidebook.md` — MPA release/deployment/issue lifecycle과 보존 경계를 추가
