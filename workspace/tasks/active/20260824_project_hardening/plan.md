...
| 3 | [Release audit hardening](sub_04_release_audit_hardening.md) | package 검증·hook 정적 검사·민감 경로 노출 방지 | 1·2단계 검증 결과 |
...
- 외부 파일 권한은 실제 사용 사례가 나타난 새 task에서만 설계한다. 현재 task에서는 MPA 관리 디렉터리 symlink 차단만 유지한다.
...
- symlink 검사만 추가하고 backup/rollback 보조 경로가 빠진다 → Step 1의 runtime/config/workspace/docs/backups 거부 회귀 테스트로 처리한다.
...
- [x] 외부 입력 권한 기능 폐기 결정 — 실제 사용 사례가 생길 때 별도 critical 작업으로 재설계
...
| 외부 파일 권한 기능 | 현재 제품에 호출 경로가 없는 선행 설계 | 폐기. 실제 사용 사례가 생기면 새 critical task로 설계 |

**파생된 작업 항목:**
- [Release audit hardening](sub_04_release_audit_hardening.md) — 남은 package·hook·receipt 검증을 독립 단위로 분리
- [CI regression automation](sub_05_ci_regression_automation.md) — 폐기. release package 생성 시 검증으로 흡수
