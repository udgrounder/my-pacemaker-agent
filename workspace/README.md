...
- **tasks vs docs:** `tasks/`는 작업 계획·진행 상태·완료 이력을 관리하고, 루트 `docs/`는 개발에 필요한 문서와 진행 과정에서 생성된 문서를 보관한다. 루트 `docs/`는 프로젝트 사용자가 소유한다.
...
agent가 작업의 실패 비용을 판단해 흐름(minor/major)을 고르고, 게이트를 거쳐 진행한다. 자세한 운영 방식은 이 설치본을 제공한 마스터 저장소의 가이드북에서 확인한다. 저장소 위치를 모르면 agent에게 가이드북 위치를 물어보면 된다.
