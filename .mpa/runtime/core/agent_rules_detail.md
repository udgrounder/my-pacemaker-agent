...
**트리거:** 세션 시작 메시지에 "⚠️ 프론트매터 없음/누락 필드" 표시가 있거나, code_gate.py가 "구현 승인 기록 복구 필요"를 출력하거나, `plan_hash.py audit` 결과 missing/invalid가 있을 때.
...
code_gate.py가 "구현 승인 기록 복구 필요" 또는 "구현 재승인 필요"를 출력했을 때 사용한다.

**승인해시 없음 복구:**
- **사용자 승인 이력 불명확** → `설계 완료`로 되돌리고 plan.md 검토 → 사용자 재승인 → `approve`
- **직전 승인 후 기록만 누락** → 명시적 사용자 승인 이력과 그때 승인된 명세를 현재 명세·위험과 대조한다. 동일함이 증명되면 근거·복구 이유를 기록하고 `설계 완료`로 복구한 뒤 정상 `approve`를 실행한다. 이미 유효한 승인을 다시 묻지 않는다. 승인된 명세를 복원할 수 없거나 차이가 있으면 승인 이력 불명확/명세 변경 경로를 따른다. hook의 복구 안내는 승인해시 직접 입력을 허용하는 뜻이 아니다.
- **minor 자동 승인 태스크** → 최소 plan.md가 사용자 요청과 일치하는지 확인 → `approve`

**해시 불일치 복구:**

1. `승인해시`가 `reqspec-v1:`로 시작하는 새 형식 plan이면 달라진 `## 요구사항 명세` 항목과 위험 신호를 먼저 확인한다. 명세 밖 변경은 `## 실행 중 변경 기록`에 남기고 누적 보고한다.
...
[hold]   태스크명3 — 보류 중
```
목록이 없으면: "진행 중인 태스크가 없습니다. 새 태스크를 시작할까요?"
...
> **INDEX 등록:** `domains/` 또는 `roles/`에 파일을 새로 만들거나 갱신했으면, `workspace/memory/INDEX.md`의 해당 섹션 행을 함께 등록·갱신한다 (생성 = 등록 동기화). **INDEX 파일이 없으면** `templates/memory_index_template.md`를 복사해 만든 뒤 등록한다. `shared/`는 INDEX에 등록하지 않는다 — 고정 이름으로 직접 로드되기 때문이다.
...
> 역할 메모리 파일을 새로 만들거나 갱신했으면 `workspace/memory/INDEX.md`의 `roles/` 표 행을 함께 등록·갱신한다.

---

## 다음 행동 제안

작업이 마무리될 때 사용자의 다음 행동을 자연스럽게 제안한다. 강요하지 않고 한 줄로. 새 태스크 생성은 가능한 다음 행동 중 하나이며, 검증 시작·재구현·중단과 구분한다.

| 상황 | 제안 |
|------|------|
| plan.md 완성, 결정 필요 항목 있음 | 미해결 결정·영향·권장안을 묶어 확인. 유효한 기존 답변은 재사용 |
| plan.md 완성, 비평 기준 해당 | `inject/_agent_execution_priority.md`에 따라 비평 진행; 별도 시작 질문 없음 |
| plan.md 완성, 비평 기준 미해당 | 필요한 구현 승인만 확인. 유효한 승인 또는 minor 실행 요청은 재사용 |
| 구현 단계 종료 | 에이전트 검증(`검증 중`)으로 자동 전이 후 사용자 테스트(`테스트 중`) 안내 |
| 테스트 완료 (이슈 있음) | 승인 범위 내 수정·재검증은 진행. 명세·위험 변경만 확인 |
...
| 수정 후 동기화 | 불필요 | source Runtime을 `sync-runtime`으로 `dist/.mpa/runtime/`에 동기화. 설치 대상 갱신은 사용자의 명시적 릴리즈·배포 요청과 immutable bundle 절차에서만 수행 |
...
1. **요청·위험 확인** — 이미 명확한 실행 요청과 작업 분류 판단 근거가 있으면 목적·짧은 계획을 알리고 진행한다. 중복 "진행할까요?"를 묻지 않는다. 계획만 요청하면 계획 작성·등록까지만 하고 `approve`·구현은 하지 않는다. 실제 의도·위험이 불명확하면 안전한 조사로 해소하고 사용자 결정이 필요한 공백만 영향·권장안과 함께 묻는다. 공통 원칙은 `core/agent_rules.md`의 "사용자 부담 최소화"를 따른다.
2. **plan.md 작성** — 반드시 공통 `templates/plan_template.md`를 Read하여 작성한다. minor도 major와 같은 `# 작업 계획서` 및 `## 요구사항 명세` 필드(요청 기준·목적·범위·제외 범위·완료 기준·사용자 결정·변경 불가 제약·가정·결정 대기·작업 분류 판단 근거)를 간결하게 채운다. minor의 분류 판단 근거에는 한 파일/단일 관심사, 설계 결정 불필요, 즉시 복구 가능, 사용자 취향·의사결정 불필요의 네 근거를 모두 쓴다. 경량화 대상은 실행·검증 절차다. minor에서는 `### 사전 조사`, `### 예상 조용한 결정`, `### 수정 대상 파일`, `### 참고 파일`, `### 반례`, `## 검증 결과`, `## 실행 중 변경 기록`, `### 구현 후 발견`을 필요 없으면 생략한다. 생략한 섹션은 자동 `approve` 전에 `### 생략 항목과 사유` 표에 조건과 작업 사실을 한 행으로 기록한다. 간결한 구현 단계와 `## 실행 TODO`, `## 운영 시 안내 사항`은 유지한다. 공통 자기 점검과 최소 확인에서 표의 사유가 실제 조건과 맞는지 확인하며 별도 비평 문서를 만들지 않는다.
...
4. **실행 요청이 있는 경우 자동 `approve` 실행** — 유효한 기존 승인은 재사용하며 같은 확인·명령을 반복하지 않는다:
   ```bash
   python3 .mpa/runtime/hooks/plan_hash.py approve workspace/tasks/active/yyyymmdd_[태스크명]/plan.md
   ```
5. **최소 확인** — 별도 검증 단계·문서 없이 변경에 맞는 확인을 수행하고 plan의 실행 TODO 아래에 방법·결과·근거를 짧게 남긴다. 문구는 diff/내용 확인, 설정은 파싱/적용 확인, 동작 변경은 관련 기존 테스트 또는 재현으로 확인한다. 새 테스트나 전체 테스트를 일괄 강제하지 않는다. 확인 수단이 없으면 미확인 이유·남은 확인을 보고하고 통과라고 기록하지 않는다. 필수 결과를 입증하지 못했으면 구현 완료로 단정하지 않는다. 구현 중 생략한 섹션을 복원하거나 새로 생략하면 초기 `생략 항목과 사유` 표는 보존하고 `## 실행 중 변경 기록`을 다시 만들거나 갱신해 대상 섹션·이전 판단·새 판단·근거·명세 영향을 기록한다. `확인 필요` 또는 완료 증거 축소이면 변경 실행 전 사용자 확인과 `renew-spec`을 따르며, 그 외에는 명세 영향 `없음`으로 기록한다.
6. 구현 완료 후 **결과·확인 근거 보고 + 완료 확인 대기** → 사용자(또는 위임 에이전트) 확인 후 에이전트가 상태·INDEX 갱신과 done 이동을 처리한다 (`agent_rules.md` "작업 항목 완료" 참조). 사용자에게 파일 이동·기록 관리를 맡기지 않는다.
7. minor에서는 changelog.md, review_phase*.md, 역할 메모리, docs 업데이트, Layer 2 현황 표시는 기본 생략한다. 외부 동작이 바뀌면 fast-path를 잠시 멈춰 실패비용을 재판정한다. 요청 안의 가역적 수정이고 아래 minor 조건을 모두 충족하며 공개 계약·보안·외부 연동 영향이 없으면 응답 대기 없이 재개하고 다음 의미 있는 보고에 묶는다. 예상 밖 범위·계약·위험 변화나 불확실성이 남으면 해당 구현을 중단하고 major/critical 전환 및 필요한 범위·승인을 확인한다. 필요한 프로젝트 문서는 전환 판단과 함께 갱신한다.
...
> **트리거:** 새 태스크 생성 판단 직전 → 반드시 즉시 읽는다.

**실패 비용 추정 — 아래 순서대로 판단한다. 불명확하면 No로 처리한다.**

**① 즉시 복구 가능한가?**
git reset으로 되돌릴 수 있고, 영구 저장(DB·외부 서비스·파일 시스템 부작용)이 없다.
→ No면 `critical`으로 올라갈 수 있음 (다음 질문으로)

**② 보안·금융·법규·외부 연동에 닿는가?**
인증, 결제, 개인정보, 외부 API 호출, 공개 인터페이스 변경 등.
→ Yes면 `critical`

**③ minor 조건을 넷 다 명확히 충족하는가?**
- 변경이 한 파일 또는 단일 관심사에 국한된다 — 불확실하면 관련 파일을 먼저 Read한 뒤 판단. 확인 후에도 불확실하면 No
- 구현 전 설계 결정이 필요 없다 — 방법이 자명하지 않으면 No
- git reset으로 즉시 복구된다 — 외부 상태(DB·파일시스템·외부 API) 변경이 있으면 No
- 사용자의 취향·의사결정이 거의 필요 없다 — 방향·스타일·우선순위 선택이 필요하면 No

| 조건 | 등급 |
|------|------|
| ② Yes (보안·외부 연동) 또는 ① No (비가역) | `critical` — 구현 중 단계별 확인 필요 |
| ③ 넷 다 명확히 Yes | `minor` — 완료 후 간략 보고 |
...
> **트리거:** 완료 처리 판단 직전 → 반드시 즉시 읽는다. minor·major 공통.

> **완료로 인정:** 이 태스크의 결과를 확인했고 끝내도 된다는 의사가 명시적인 발화 ("완료", "됐어", "확인했어" 등)
> **에이전트가 먼저 완료를 제안한 경우에만:** 짧은 긍정 발화도 인정 ("응", "맞아", "그래")
> **완료로 인정하지 않음:** 태스크 종료 선언 없이 다음으로 넘어가려는 발화 ("넘어가자", "다음으로", "계속해", "처리해줘" 등)
> **모호한 경우:** 한 번만 확인 질문 → "이 태스크를 완료 처리할까요, 아니면 계속 진행할까요?"
> **major 전용:** 에이전트는 완료 처리를 제안할 수 있다. 단, 사용자 승인 없이 완료 처리하지 않는다.
