...
| `deploy` | recorded dry-run, 승인·rollback 책임 | target release/asset/history/receipt/issue inventory/만료 재검증 | `from_release → to_release`, 수집·원본 정리 결과 고지, Runtime backup; 없는 `workspace/`·`workspace/issues/`·`docs/INDEX.md` 생성 | 기존 workspace·docs·agent 설정·일반 소스 변경 |
