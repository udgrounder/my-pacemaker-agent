...
| `deploy` | recorded dry-run, 승인·rollback 책임 | target fingerprint·release/asset/history/receipt/issue inventory/config/만료 재검증; 배포 전 `.mpa/runtime`와 migration 대상 config backup | `from_release → to_release`, 설정 migration·수집·원본 정리 결과 고지, Runtime+config backup; 성공 후 최신 3개 유지; Git 비추적 local receipt와 target history의 credential·machine path 정제; 없는 `workspace/`·`workspace/issues/`·`docs/INDEX.md` 생성 | 기존 user config 값·workspace·docs·agent 설정·일반 소스 변경 |
| `rollback` | target 또는 등록된 target-ref, backup, release ID, 승인·책임 | target history와 backup 범위 검사, Runtime 및 config snapshot 무결성 확인. target 생략 시 로컬 등록부 fingerprint 일치 확인 | Runtime과 MPA config를 함께 rolled_back receipt/history로 복원 | backup 범위 밖 읽기·복원, 공유 receipt에 target 절대경로 기록 |
