...
| `history-cleanup` | **사용자가 명시한 이력 정리 요청**, 선택적 `--keep` 및 `--backup-keep` | 기본 dry-run으로 release bundle·등록 대상의 deployment history/receipt·성공 `runtime.zip` backup 후보를 전체 제시. `--apply`에는 승인자·승인 기록·rollback 책임자 필수 | 후보 JSON / 승인된 apply 시에만 release·history·receipt·관리형 backup 정리. registry 불일치 대상·실패 backup·사용자 snapshot은 경고·보존 | prepare-release, deploy, rollback 중 자동 정리; dry-run 없이 삭제; 등록되지 않았거나 fingerprint 불일치 대상 삭제 |
| `deployment-dry-run` | manifest, target, target-ref | release package·target fingerprint·release/history·issue inventory·config checksum/migration 후보 확인 | 만료 가능한 dry-run receipt와 수집 후보·설정 추가 후보 고지. target 절대경로는 receipt 대신 Git 비추적 로컬 등록부에만 기록 | 대상 변경 |
| `deploy` | recorded dry-run, 승인·rollback 책임 | target fingerprint·release/asset/history/receipt/issue inventory/config/만료 재검증; 배포 전 `.mpa/runtime`와 migration 대상 config backup | `from_release → to_release`, 설정 migration·수집·원본 정리 결과 고지, Runtime+config backup; 성공 시 version backup 내부에 검증된 `runtime.zip`만 남김; Git 비추적 local receipt와 target history의 credential·machine path 정제; 없는 `workspace/`·`workspace/issues/`·`docs/INDEX.md` 생성 | 기존 user config 값·workspace·docs·agent 설정·일반 소스 변경, 과거 이력 자동 정리 |
| `rollback` | target 또는 등록된 target-ref, version backup, release ID, 승인·책임 | target history와 backup 범위 검사, marker·`runtime.zip`·config snapshot 무결성 확인. target 생략 시 로컬 등록부 fingerprint 일치 확인 | Runtime과 MPA config를 함께 rolled_back receipt/history로 복원 | legacy `runtime/` backup 또는 backup 범위 밖 읽기·복원, 공유 receipt에 target 절대경로 기록 |
