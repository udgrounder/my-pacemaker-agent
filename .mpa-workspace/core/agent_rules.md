...
> **minor 판단 기준 — 넷 다 명확히 Yes여야 minor:**
...
1. **계획 제시** — 자동 진행 여부와 무관하게 **반드시** 아래 형식으로 출력한다. 생략 불가.
   - 필수 판단 질문(언어, 기능 범위 등)이 있으면 **2개 이내**로 먼저 확인한다.
   - 거부나 방향 변경이 아닌 응답이면 다음 단계로 진행한다 (Zone 3 — 명시적 승인 불필요):
   ```
   minor 태스크: [목적].
   [계획 핵심 1-2줄]
...
3. **`approve` 실행**:
   ...
   ```
4. 구현 완료 후 **보고 + 사용자 확인 대기** → 확인 후 done 처리 (아래 "작업 완료" 섹션 참조)
...
> **완료로 인정:** "완료", "완료 처리해줘", "됐어", "ok" 등 완료 의사가 명확한 발화
