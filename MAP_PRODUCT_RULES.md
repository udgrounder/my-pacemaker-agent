...
- 대상 프로젝트의 `workspace/`, `docs/`, agent 설정, 일반 소스는 Runtime deployment와 rollback에서 읽거나 변경하지 않는다. 단, 없는 `workspace/`, `workspace/issues/`, `docs/`, `docs/INDEX.md`는 초기화할 수 있다. 승인된 issue collection은 이 불변식의 유일한 좁은 예외이며 Runtime 교체와 별도 권한으로 취급한다. deployment dry-run은 대상 최상위 `workspace/issues/*.md`의 안전한 상대 식별자와 checksum·분류 metadata만 읽고, deploy commit은 dry-run receipt에 결박된 `collectable` 원본만 목적지 검증 뒤 제거할 수 있다. `tasks/`·`docs/`·`memory/` 및 하위 issue 디렉터리는 읽거나 변경하지 않는다.
...
- 이슈 수집은 원칙적으로 명시 요청 대상만 처리한다. 단, 승인된 Runtime update는 dry-run에서 후보를 고지한 뒤 검증 성공 시에만 batch를 원자 수집하고 원본 정리 결과를 고지할 수 있다. deploy는 Runtime 교체 전 receipt 전체를 재검증하고, 교체한 Runtime 검증과 config migration 뒤 issue batch를 commit한다. issue collection 또는 이후 backup·receipt 기록이 실패하면 이동한 issue와 Runtime·MPA config를 함께 원복한다.
