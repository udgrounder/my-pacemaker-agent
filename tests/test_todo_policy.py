...
class TodoPolicyTest(unittest.TestCase):
    def assert_policy(self, workspace):
        ...
        self.assertIn("이번 태스크 종료 전 증빙", major)
        self.assertIn("구현·설계·검증이 추가로 필요한 후속 작업은 새 태스크", major)
        self.assertIn("사용자·운영자가 수행할 행동", major)
        self.assertIn("이번 태스크 종료 전 증빙", minor)
        self.assertIn("구현·설계·검증이 추가로 필요한 후속 작업은 새 태스크", minor)
        self.assertIn("이번 태스크가 끝나기 전 증빙", rules)
        self.assertIn("구현·설계·검증이 추가로 필요한 후속 작업은 별도 태스크", rules)
