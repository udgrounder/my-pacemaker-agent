...
9. **plan.md 상태를 `검증 중`으로 업데이트한다** — 다음은 에이전트 검증 세션(`layer1_review.md`).
   > 검증은 **서브에이전트로 격리 실행**한다 (구현 컨텍스트 격리). 메인 에이전트는 검증을 수행하지 않고 오케스트레이션만 한다.
   > 서브에이전트 도구가 없는 환경에서는 자가 검증으로 대체. 상세: `inject/layer1_review.md` "실행 환경".
...
11. **다음 작업 예측:** *"에이전트 검증을 시작합니다."* — 자동으로 `검증 중`으로 전이.
