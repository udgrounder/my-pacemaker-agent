...
7. **다음 작업 예측:**
   - "사용자 결정 필요" 항목이 있으면 → *"사용자 결정 필요 항목을 먼저 검토할까요?"*
   - 결정 항목 없고 계획 복잡도가 높으면 → *"독립 비평(layer1_critique)을 먼저 진행할까요? 새 스레드에서 plan.md만 보고 약점을 찾아드립니다."*
   - 결정 항목 없고 계획이 단순하면 → *"바로 구현을 진행할까요?"*
