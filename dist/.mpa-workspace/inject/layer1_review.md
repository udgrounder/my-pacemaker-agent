...
2. 역할 메모리 업데이트 → 놀라움 필터 적용 후 `workspace/memory/roles/code_reviewer.md`에 기록 (기억할 것이 없으면 생략)
3. 하네스 개선 후보 발견 → `.mpa-workspace/upgrade-candidates/[내용].md`
4. **다음 작업 예측:**
   - 🚨 즉시 수정 필요 항목이 있으면 → *"발견된 이슈를 수정할까요?"*
   - 이슈가 없으면 → 사용자에게 묻는다: *"결과물을 직접 써보셨나요? 수정하거나 추가하고 싶은 게 있으시면 `@.mpa-workspace/inject/layer1_discovery.md`로 함께 정리할 수 있습니다."*
   - 발견 사항 없음 → *"태스크를 완료 처리할까요?"*
