...
**단계 모델 — minor (경량, 3단계):**
```
작성 중 [자동 승인] → 구현 중 → 완료 승인 ⛔G2 → done
```
- **GATE 1 생략**: 에이전트가 최소 plan.md 작성 직후 즉시 `approve` 실행. 사용자 명시적 승인 불필요.
- **GATE 2**: 구현 완료 후 보고 → 사용자(또는 위임 에이전트) 확인 후 done 처리. 자동 완료 없음.
...
**minor 태스크 경량 처리 (GATE 1 생략 / GATE 2 경량 확인):**
...
- ⚠️ 복구 시 에이전트가 설계 영향 여부를 먼저 분류하지 않는다 — 변경 내용 제시 → 사용자 분류 → 실행 순서 고정. 상세: `agent_rules_detail.md` "GATE 1 재진입 복구" 섹션.
...
> **공통 원칙 — 완료 보고의 증거 기반:** "완료했다"고 보고할 때는 서술만으로 주장하지 않는다. 실제 변경 근거(수정된 파일의 핵심 줄 인용, 실행한 명령의 출력, 통과한 테스트 결과 등) 중 최소 하나를 인용해 보고에 포함한다. 인용할 근거가 없다면 작업이 실제로 끝나지 않았다는 신호다.

**minor 태스크 — ⛔ GATE 2: 사용자(또는 위임 에이전트) 확인 후 done 처리**
...
> **완료로 인정:** 이 태스크가 끝났다는 의사가 명시적인 발화 ("완료", "됐어", "ok", "확인했어", "좋아", "처리해줘" 등)
> **에이전트가 먼저 완료를 제안한 경우에만:** 짧은 긍정 발화도 인정 ("응", "맞아", "그래")
> **완료로 인정하지 않음:** 태스크 종료 선언 없이 다음으로 넘어가려는 발화 ("넘어가자", "다음으로", "계속해" 등)
