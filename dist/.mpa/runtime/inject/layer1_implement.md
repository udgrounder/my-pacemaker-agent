...
plan.md YAML 프론트매터의 `실패비용`이 `minor`이면 먼저 `상태: 구현 중`과 유효한 `reqspec-v1:` 승인해시를 확인한다. 둘 중 하나라도 없으면 구현·최소 확인을 시작하지 않고 `core/agent_rules_detail.md`의 "minor 경량 처리 절차"로 돌아가 계획만 요청인지, 명확한 실행 요청인지 판별한다. 명확한 실행 요청이면 plan 작성·자동 `approve` 후 이 fast-path로 돌아온다. 최소 확인·위험 재판정·생략 범위는 해당 절차가 정본이다.
